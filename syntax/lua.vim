...
"syn match luaExtendedOp /\%(\S\s*\)\@<=[+\-*/%&|\^=!<>?.]\{-1,}\|[-=]>\|:/ display

" catch errors caused by wrong parenthesis and wrong curly brackets or
" keywords placed outside their respective blocks
syn region luaParen      matchgroup=luaDelimiter start='(' end=')' contains=ALLBUT,luaParenError,luaTodo,luaSpecial,luaIfThen,luaElseifThen,luaElse,luaThenEnd,luaBlock,luaLoopBlock,luaIn,luaStatement
syn region luaTableBlock matchgroup=luaDelimiter start="{" end="}" contains=ALLBUT,luaBraceError,luaTodo,luaSpecial,luaIfThen,luaElseifThen,luaElse,luaThenEnd,luaBlock,luaLoopBlock,luaIn,luaStatement

syn match  luaParenError ")"
syn match  luaBraceError "}"
"syn match  luaError "\<\%(end\|else\|elseif\|then\|until\|in\)\>"
syn match  luaError "\<\%(else\|elseif\|then\|until\|in\)\>"
...
" faoating point number, without dot, with exponent
syn match luaNumber  "\<\d\+[eE][-+]\=\d\+\>"
...
syn match luaDelimiter          /[(){}]/ display

syn match luaSpecialOp /[,;]/ display

syn match luaConstant /\<\u[A-Z0-9_]\+\>/ display
syn match luaDotAccess /\([^.]\.\)\@<=[a-z]\w\+/ contains=@luaIdentifier

"luaBlock, luaBraceError, luaComment, luaCond, luaConstant, luaDelimiter,
"luaDotAccess, luaElse, luaError, luaExtendedOp, luaFor, luaFunc,
"luaFunction, luaIdentifier, luaIn, luaInnerComment, luaLabel, luaNumber,
"luaOperator, luaParenError, luaRepeat, luaSpecial, luaSpecialOp, luaStatement,
"luaString, luaString2, luaTable, luaThenEnd, luaTodo
...
if version >= 508 || !exists("did_lua_syntax_inits")
  ...
  HiLink luaExtendedOp		Operator
  HiLink luaSpecialOp           Special
  HiLink luaDelimiter		Special
  ...
  HiLink luaStringDelimiter	StringDelimiter
  HiLink luaNumber		Number
  HiLink luaConstant		Constant
  "HiLink luaTable		Structure
  HiLink luaTableBlock		StorageClass

  "HiLink luaFunc		PredefinedIdentifier
  HiLink luaFunc		Type
  HiLink luaSpecial		SpecialChar
  HiLink luaLabel		Label

  HiLink luaDotAccess           Function
