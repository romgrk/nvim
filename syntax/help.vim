...
"syn match helpHeadline		"\v^\s*[A-Z0-9. ]+\.[ \t]*[^.|`*]+"
syn match helpSectionDelim	"^===.*===$"
syn match helpSectionDelim	"^---.*--$"
syn match helpSectionDelim	"\.{5,}"
if has("conceal")
...
syn match helpNote		/\v\c^\s*notes?:?/
syn match helpImportant	        /\v\c^\s*important:?/
...
hi def link helpHyperTextJump	Special
hi def link helpBar		Ignore
hi def link helpBacktick	Ignore
hi def link helpStar		Ignore
hi def link helpHyperTextEntry	String
hi def link helpHeadline	Title
hi def link helpHeader		PreProc
hi def link helpSectionDelim	Delimiter
hi def link helpVim		Statement
hi def link helpCommand		Statement
hi def link helpExample		Comment
hi def link helpOption		Type
hi def link helpSpecial		OldSpecial
hi def link helpNote		Todo
hi def link helpImportant       Debug
...
hi def link helpURL		Underlined
