...
local function statusline_lsp()
  ...
  if diagnostics.errors and diagnostics.errors > 0 then
    table.insert(status_parts, vim.g.icons.by_name.error .. ' ' .. diagnostics.errors)
    only_hint = false
    some_diagnostics = true
  end

  if diagnostics.warnings and diagnostics.warnings > 0 then
    table.insert(status_parts, vim.g.icons.by_name.warning .. ' ' .. diagnostics.warnings)
    only_hint = false
    some_diagnostics = true
  end

  if diagnostics.info and diagnostics.info > 0 then
    table.insert(status_parts, vim.g.icons.by_name.info .. ' ' .. diagnostics.info)
    only_hint = false
    some_diagnostics = true
  end

  if diagnostics.hints and diagnostics.hints > 0 then
    table.insert(status_parts, vim.g.icons.by_name.hint .. ' ' .. diagnostics.hints)
    some_diagnostics = true
  end
  ...
  return symbol .. vim.g.icons.by_name.ok .. ' '
end
