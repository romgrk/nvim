...
function! s:DoSyscallCommon(syscall, CallFunc, throw_errors) abort
  call ensure#IsBool(a:throw_errors)
  let l:return_data = {}
  ...
  " instead.
  let l:shell_state = value#SaveAll(['&shell', '$SHELL'])
  ...
  try
    let l:return_data = function#Apply(a:CallFunc)
  finally
    " Restore configured shell.
    call value#Restore(l:shell_state)
  endtry
  ...
  throw error#Message('ShellError', l:err_msg, a:syscall.GetCommand())
endfunction
...
function! syscall#DoCall() abort dict
...
function! syscall#IsAsyncAvailable() abort
...
function! syscall#DoCallAsync() abort dict
  let l:error_file = tempname()
  ...
  let s:callbacks[l:output_file] = {
      \ 'function': ensure#IsCallable(self.callback),
      \ 'env': s:CurrentEnv()}
...
function! syscall#DoCallForeground(pause) abort dict
  let l:return_data = {}
  if a:pause
...
" convenience.
" @throws WrongType
function! syscall#Create(cmd) abort
  if value#IsDict(a:cmd)
    return a:cmd
  endif
  return {
      \ 'cmd': ensure#TypeMatchesOneOf(a:cmd, ['', []]),
      \ 'WithCwd': function('syscall#WithCwd'),
      \ 'WithStdin': function('syscall#WithStdin'),
      \ 'And': function('syscall#And'),
      \ 'Or': function('syscall#Or'),
      \ 'Call': function('syscall#Call'),
      \ 'CallAsync': function('syscall#CallAsync'),
      \ 'CallForeground': function('syscall#CallForeground'),
      \ 'GetCommand': function('syscall#GetCommand')}
endfunction
...
function! syscall#WithCwd(directory) abort dict
  ...
  if !isdirectory(l:directory)
    throw error#NotFound('Directory %s does not exist.', l:directory)
  endif
...
" @throws WrongType
function! syscall#WithStdin(input) abort dict
  let l:new_cmd = copy(self)
  let l:new_cmd.stdin = ensure#IsString(a:input)
  return l:new_cmd
endfunction
...
" @throws WrongType
function! syscall#And(cmd) abort dict
  let l:cmd_string = syscall#Create(a:cmd).GetCommand()
  let l:new_cmd = copy(self)
...
" {cmd} may be any valid @function(#Create) argument.
" @throws WrongType
function! syscall#Or(cmd) abort dict
  let l:cmd_string = syscall#Create(a:cmd).GetCommand()
  let l:new_cmd = copy(self)
  let l:new_cmd.cmd = join([self.GetCommand(), l:cmd_string], ' || ')
  return l:new_cmd
...
function! syscall#Call(...) abort dict
  let l:throw_errors = ensure#IsBool(get(a:, 1, 1))
  let l:call_func = function#Create('syscall#DoCall', [], self)
...
"       call error#Shout('sleep command failed: %s', a:result.stderr)
"       return
"     endif
"     echomsg 'Success!'
"   endfunction
"   call syscall#Create(['sleep', '3']).CallAsync('Handler', 1)
" <
...
function! syscall#CallAsync(Callback, allow_sync_fallback) abort dict
  let self.callback = ensure#IsCallable(a:Callback)
  call ensure#IsBool(a:allow_sync_fallback)
  if !syscall#IsAsyncAvailable()
    if a:allow_sync_fallback
      ...
      call function#Call(self.callback, [s:CurrentEnv(), l:return_data])
      return
    else
      " Neither async nor sync is available. Throw error with salient reason.
      if s:async_disabled
        let l:reason = 'disabled by syscall#SetAsyncDisabled.'
  ...
  let l:call_func = function#Create(
      \ 'syscall#DoCallAsync', [], self)
  ...
  try
    ...
    throw error#Failure('Problem dispatching async syscall: %s',
        \ error#Split(v:exception)[1])
  endtry
endfunction
...
function! syscall#CallForeground(pause, ...) abort dict
  let l:throw_errors = ensure#IsBool(get(a:, 1, 1))
  if !has_key(self, 'stdin')
    let l:call_func = function#Create(
        \ 'syscall#DoCallForeground', [a:pause], self)
    return s:DoSyscallCommon(self, l:call_func, l:throw_errors)
  endif
...
function! syscall#GetCommand() abort dict
  if value#IsString(self.cmd)
    " Accept strings for convenience, return as-is.
    return self.cmd
  endif
  let l:words = map(copy(self.cmd), 'string#Strip(v:val)')
...
" sh-incompatible) as that will break all plugins using |syscall| and
" expecting sh syntax.
"
...
function! syscall#SetUsableShellRegex(regex) abort
...
function! syscall#SetAsyncDisabled(disabled)
...
function! syscall#AsyncDone(stdout_file, stderr_file, exit_code)
  ...
  call function#Call(l:callback_info.function,
