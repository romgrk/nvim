...
function! s:Reverse(dict) abort
  ...
  for [l:key, l:Val] in items(a:dict)
    ...
    for [l:Seen, l:by] in l:reversed
      if value#IsEqual(l:Val, l:Seen)
        let l:msg = 'Value %s not unique in enum. (Assigned to both %s and %s).'
        throw error#BadValue(l:msg, l:Val, l:key, l:by)
      endif
    endfor
...
" This can be fooled, but only if someone is accessing private enum
" functions, in which case they can't blame us.
function! enum#IsValid(Value) abort
  return value#IsDict(a:Value) &&
      \ has_key(a:Value, 'Name') &&
      \ value#IsEqual(a:Value.Name, function('enum#Name'))
endfunction
...
" >
"   let g:animals = enum#Create(['DUCK', 'PIG'])
...
"     not strings.
function! enum#Create(names) abort
  if empty(a:names)
    throw error#BadValue('Enum must have at least one name.')
  endif
  call ensure#IsCollection(a:names)
  if value#IsList(a:names)
    let l:enum = {}
    let l:counter = 0
    for l:name in a:names
      call s:EnsureEnumName(l:name)
      if has_key(l:enum, l:name)
        throw error#BadValue('%s appears in enum twice.', l:name)
      endif
  ...
  let l:enum.Name = function('enum#Name')
  let l:enum.Names = function('enum#Names')
  let l:enum.Value = function('enum#Value')
  let l:enum.Values = function('enum#Values')
  return l:enum
endfunction
...
" >
"   let g:animals = enum#Create(['DUCK', 'PIG', 'COW'])
"   echomsg g:animals.PIG      " This will echo 1.
...
function! enum#Name(Value) dict abort
  for [l:Val, l:key] in self._lookup
    if value#IsEqual(l:Val, a:Value)
      return l:key
    endif
    " May change type.
    unlet l:Val
  endfor
  throw error#NotFound('Enum name with value %s.', a:Value)
endfunction
...
function! enum#Names() dict abort
...
function! enum#Value(name) dict abort
  call s:EnsureEnumName(a:name)
  if has_key(self, a:name)
    return self[a:name]
  endif
  throw error#NotFound('Enum name %s.', a:name)
endfunction
...
function! enum#Values() dict abort
