"" Deprecated system call API. See syscall for the new API.
...
" Functions in system# pass any system command arguments to this
" function, allowing you to use lists or escaped strings anywhere.
function! system#Command(words) abort
  return syscall#Create(a:words).GetCommand()
endfunction
...
function! system#Call(cmd, ...) abort
  let l:execution = ensure#IsString(get(a:, 1, ''))
  if empty(l:execution)
    let l:execution = s:default_context
  endif
  call ensure#IsIn(l:execution, s:contexts)
  let l:cmd = syscall#Create(a:cmd)
  if l:execution ==# 'hidden'
    let l:cmd_fn = function#Method(l:cmd, 'Call')
  elseif l:execution ==# 'foreground'
    let l:cmd_fn = function#Method(l:cmd, 'CallForeground').WithArgs(0)
  elseif l:execution ==# 'foreground_pause'
    let l:cmd_fn = function#Method(l:cmd, 'CallForeground').WithArgs(1)
  else
...
" @throws ShellError if the shell command returns an exit code.
function! system#CallAt(cmd, working_dir, ...) abort
  let l:cmd = syscall#Create(a:cmd).WithCwd(a:working_dir)
  return call('system#Call', [l:cmd] + a:000)
endfunction
...
function! system#And(cmds) abort
  let l:new_cmd = syscall#Create(a:cmds[0])
...
""
" @deprecated Use @function(syscall#SetUsableShellRegex).
" @private
...
function! system#SetUsableShellRegex(regex) abort
  call syscall#SetUsableShellRegex(a:regex)
endfunction
