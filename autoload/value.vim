...
function! s:GetFocus(target, focus) abort
  call ensure#TypeMatchesOneOf(a:target, [[], {}])
  if value#IsList(a:target)
    call ensure#IsNumber(a:focus)
    if a:focus >= len(a:target)
      let l:msg = 'Index %s out of range in %s'
      throw error#BadValue(l:msg, a:focus, string(a:target))
    endif
...
function! s:SetFocus(target, focus, Value) abort
  call ensure#IsCollection(a:target)
  if !value#IsDict(a:target)
    call ensure#IsNumber(a:focus)
    if a:focus > len(a:target)
      let l:msg = 'Index %s out of range in %s'
      throw error#BadValue(l:msg, a:focus, string(a:target))
...
function! value#IsEqual(X, Y) abort
...
function! value#IsIn(Value, list)
  return index(ensure#IsList(a:list), a:Value) >= 0
endfunction
...
function! value#EmptyValue(Value) abort
...
" See also |type()|.
function! value#TypeName(Value) abort
  let l:type = type(a:Value)
  if l:type == 0
    return "number"
  elseif l:type == 1
...
function! value#TypeMatches(Value, Reference) abort
...
" 0 otherwise.
function! value#TypeMatchesOneOf(Value, references) abort
...
function! value#IsNumber(Value) abort
...
function! value#IsString(Value) abort
...
function! value#IsFuncref(Value) abort
...
function! value#IsList(Value) abort
  return type(a:Value) == type([])
endfunction
...
function! value#IsDict(Value) abort
  return type(a:Value) == type({})
endfunction
...
function! value#IsFloat(Value) abort
...
" 0 otherwise.
function! value#IsNumeric(Value) abort
  return value#TypeMatchesOneOf(a:Value, [0, 0.0])
endfunction
...
" 0 otherwise.
function! value#IsCollection(Value) abort
  return value#TypeMatchesOneOf(a:Value, [[], {}])
endfunction
...
" exists.
function! value#IsCallable(Value) abort
  return value#TypeMatchesOneOf(a:Value, ['', s:EmptyFn])
      \ || function#IsWellFormedDict(a:Value)
endfunction


""
" 1 if {value} is a maktaba enum type, 0 otherwise.
function! value#IsEnum(Value) abort
  return enum#IsValid(a:Value)
endfunction
...
" >
"   value#Focus({'a': [0, {'b': 'hi!'}, 1]}, ['a', 1, 'b']) == 'hi'
" <
...
function! value#Focus(Target, foci, ...) abort
  call ensure#IsList(a:foci)
  if a:0 == 0
    ...
    for l:focus in a:foci
      try
        ...
        throw error#WrongType(l:msg, string(l:focus), string(l:Target))
      endtry
    ...
    let l:parent = value#Focus(a:Target, a:foci[:-2])
...
" @throws WrongType
" @throws BadValue
function! value#Save(variable) abort
  return value#SaveAll([a:variable])
endfunction
...
" @throws WrongType
" @throws BadValue
function! value#SaveAll(variables) abort
  let l:savedict = {}
  for l:name in ensure#IsList(a:variables)
    call ensure#IsString(l:name)
    if string#StartsWith(l:name, '$')
      " Capture environment variable.
      " Use eval() since expand() has different behavior (see
      " :help expr-env-expand).
      call ensure#Matches(l:name, s:ENV_VAR_NAME)
      let l:savedict[l:name] = exists(l:name) ? eval(l:name) : s:UNSET
    elseif string#StartsWith(l:name, '&')
      " Capture vim setting.
      call ensure#Matches(l:name, s:SETTING_NAME)
      let l:savedict[l:name] = exists(l:name) ? eval(l:name) : s:UNSET
    else
...
" @throws WrongType
" @throws BadValue
function! value#Restore(state) abort
  call ensure#IsDict(a:state)
