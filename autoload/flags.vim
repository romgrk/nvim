...
"
" The solution is flags. Each Maktaba plugin which uses |plugin#Enter|
...
" @default default=0
function! flags#Create(name, ...) abort
  " May be a funcref, must start with a capital letter.
  let l:dict = {
      \ '_name': a:name,
      \ 'Get': function('flags#Get'),
      \ 'GetCopy': function('flags#GetCopy'),
      \ 'Set': function('flags#Set'),
      \ '_callbacks': reflist#Create(),
      \ 'AddCallback': function('flags#AddCallback'),
      \ 'Callback': function('flags#Callback'),
      \ '_translators': reflist#Create(),
      \ 'AddTranslator': function('flags#AddTranslator'),
      \ 'Translate': function('flags#Translate'),
      \ }
...
function! flags#Get(...) dict abort
  return value#Focus(self._value, get(a:, 1, []))
endfunction
...
" @throws BadValue if [foci] are invalid.
" @throws WrongType if [foci] contains the wrong types to index the flag.
function! flags#GetCopy(...) dict abort
  return deepcopy(call('flags#Get', a:000, self))
endfunction
...
" @throws WrongType if [foci] contains the wrong types to index the flag.
function! flags#Set(Value, ...) dict abort
  let l:foci = ensure#IsList(get(a:, 1, []))
  if empty(l:foci)
    let l:Value = a:Value
  else
    " We do a deepcopy here because we don't want to commit changes until all
    " translators succeed.
    let l:Value = value#Focus(deepcopy(self._value), l:foci, a:Value)
  endif
...
function! flags#AddCallback(F, ...) dict abort
  call ensure#IsCallable(a:F)
  let l:fire_immediately = ensure#IsBool(get(a:, 1, 1))

  let l:remover = self._callbacks.Add(a:F)
  if l:fire_immediately
    call function#Apply(a:F, self._value)
  endif
  return l:remover
endfunction
...
function! flags#Callback() dict abort
  for l:Callback in self._callbacks.Items()
    call function#Apply(l:Callback, self._value)
    " May change type.
    unlet l:Callback
  endfor
endfunction
...
function! flags#AddTranslator(F) dict abort
  call ensure#IsCallable(a:F)
  let l:remover = self._translators.Add(a:F)
  unlockvar! self._value
  let self._value = function#Apply(a:F, self._value)
  lockvar! self._value
  call self.Callback()
  return l:remover
endfunction
...
function! flags#Translate(Value) dict abort
  let l:Value = a:Value
  if !empty(self._translators)
    for l:Translator in self._translators.Items()
      let l:Value = function#Apply(l:Translator, l:Value)
      " May change type.
      unlet l:Translator
    endfor
  endif
