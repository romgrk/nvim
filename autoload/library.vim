function! s:NotALibrary(plugin, message, ...) abort
  ...
  return error#Exception('NotALibrary', l:msg, [a:plugin.name] + a:000)
endfunction
...
function! s:EnsureIsLibrary(plugin) abort
  ...
  let l:dirs = filter(plugin#NonlibraryDirs(), 'a:plugin.HasDir(v:val)')
  if empty(l:dirs)
...
" nicely for the user. (See |error#Shout|). Otherwise the user
...
" or any other user-impacting functionality. library#Require is designed
...
"    @function(library#AddInstaller), in order of registration.
"
...
function! library#Import(library) abort
  let l:name = plugin#CanonicalName(a:library)
  if plugin#IsRegistered(l:name)
    let l:plugin = plugin#Get(l:name)
...
function! library#Require(library) abort
  try
    call library#Import(a:library)
  catch /ERROR(NotFound):/
...
function! library#AddInstaller(name, F) abort
...
function! library#RemoveInstaller(name) abort
  ...
  throw error#NotFound(l:msg, a:name)
endfunction
