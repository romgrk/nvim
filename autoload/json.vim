...
if !exists('json#NULL')
  let json#NULL = {'__json__': 'null'}
  let s:NULL = json#NULL
  let json#TRUE = {'__json__': 'true'}
  let s:TRUE = json#TRUE
  let json#FALSE = {'__json__': 'false'}
  let s:FALSE = json#FALSE
  lockvar! json#NULL json#TRUE json#FALSE
...
function! s:FormatKeyAndValue(key, value) abort
  if value#IsString(a:key)
    return printf('"%s": %s', a:key, json#Format(a:value))
  endif
...
function! json#Format(value) abort
  ...
  elseif a:value is s:FALSE
    return 'false'
  elseif value#IsNumeric(a:value)
    ...
    return l:json
  elseif value#IsString(a:value)
    let l:escaped = substitute(escape(a:value, '"\'), "\n", '\\n', 'g')
    let l:escaped = substitute(l:escaped, "\t", '\\t', 'g')
    " TODO(dbarnett): Escape other special characters.
    return '"' . l:escaped . '"'
  elseif value#IsList(a:value)
    let l:json_items = map(copy(a:value), 'json#Format(v:val)')
    return '[' . join(l:json_items, ', ') . ']'
  elseif value#IsDict(a:value)
    if value#IsCallable(a:value)
      throw error#BadValue(
  ...
  throw error#BadValue(a:value)
endfunction

function! s:Consume(str, count) abort
  return string#StripLeading(a:str[a:count : ])
endfunction
...
function! s:ParsePartial(json, custom_values) abort
  ...
  throw error#BadValue('Input is not valid JSON text.')
endfunction
...
" use instead of json# sentinels. For example: >
"   let value = json#Parse('[null]', {'null': ''})
" <
...
function! json#Parse(json, ...) abort
  let l:json = string#Strip(ensure#IsString(a:json))
  let l:custom_values = ensure#IsDict(get(a:, 1, {}))
