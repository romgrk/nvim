...
" {message} will be formatted with [args...], as in |printf()|.
function! error#Warn(message, ...) abort
...
function! error#Shout(message, ...) abort
...
" >
"   function! error#NotFound(message, ...)
"     return error#Exception('NotFound', a:message, a:000)
"   endfunction
" <
"
...
function! error#Exception(type, message, fmtargs) abort
  call ensure#Matches(a:type, s:errortype)
  if empty(a:fmtargs)
...
" @throws BadValue if {type} contains invalid characters.
" @throws WrongType if {type} is not a string.
function! error#Message(type, message, ...) abort
  return error#Exception(a:type, a:message, a:000)
endfunction
...
function! error#NotAuthorized(message, ...) abort
  return error#Exception('NotAuthorized', a:message, a:000)
endfunction
...
function! error#NotFound(message, ...) abort
  return error#Exception('NotFound', a:message, a:000)
endfunction
...
function! error#NotImplemented(message, ...) abort
  return error#Exception('NotImplemented', a:message, a:000)
endfunction
...
function! error#WrongType(message, ...) abort
  return error#Exception('WrongType', a:message, a:000)
endfunction
...
function! error#BadValue(message, ...) abort
  return error#Exception('BadValue', a:message, a:000)
endfunction
...
function! error#InvalidArguments(message, ...) abort
  return error#Exception('InvalidArguments', a:message, a:000)
endfunction
...
function! error#MissingFeature(message, ...) abort
  return error#Exception('MissingFeature', a:message, a:000)
endfunction
...
function! error#Failure(message, ...) abort
  return error#Exception('Failure', a:message, a:000)
endfunction
...
function! error#Split(exception) abort
  ...
  if empty(l:match)
    ...
    if empty(l:match)
      ...
      throw error#BadValue(l:err, a:exception)
    endif
...
function! s:ExceptionMatches(exceptions, errortext) abort
  if value#IsString(a:exceptions)
    return v:exception =~# a:exceptions
  endif
  for l:exception in a:exceptions
    if value#IsNumber(l:exception) || l:exception =~# '\v^\d+$'
...
"
" {func} may be any maktaba callable. See |function#Create|.
"
...
" equivalent:
" >
"   call error#Try(g:fn, 'ERROR(BadValue):\|E107:')
"   call error#Try(g:fn, ['BadValue', 107])
" <
...
"     regex nor a list.
function! error#Try(F, ...) abort
  let l:exceptions = ensure#TypeMatchesOneOf(get(a:, 1, '.*'), [[], ''])
  if value#IsList(l:exceptions)
    call map(l:exceptions, 'ensure#TypeMatchesOneOf(v:val, [0, ""])')
  endif
...
function! error#TryCommand(command, ...) abort
  let l:exceptions = ensure#TypeMatchesOneOf(get(a:, 1, '.*'), [[], ''])
