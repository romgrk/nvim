...
function! list#RemoveItem(list, item) abort
  let l:index = index(a:list, a:item)
  if l:index < 0
    let l:msg = 'Item %s in list %s'
    throw error#NotFound(l:msg, string(a:item), string(a:list))
  endif
...
" {list} is returned for convenience.
function! list#RemoveDuplicates(list) abort
...
" {list} is modified in place and returned for convenience.
function! list#RemoveAll(list, item) abort
  let l:index = index(a:list, a:item)
  while l:index >= 0
    call remove(a:list, l:index)
    let l:index = index(a:list, a:item)
