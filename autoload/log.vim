...
function! s:DoMessage(level, context, message, ...) abort
  call ensure#IsNumber(a:level)
  call ensure#IsString(a:context)
  call ensure#IsString(a:message)
  if len(a:000)
...
function! s:SendToHandlers(logitem) abort
  ...
  for l:Handler in l:maktaba.globals.loghandlers.Items()
    call function#Call(l:Handler, a:logitem)
  endfor
endfunction
...
function! log#Logger(context) abort
  return {
      \ '_context': a:context,
...
function! log#Debug(message, ...) dict abort
...
function! log#Info(message, ...) dict abort
...
function! log#Warn(message, ...) dict abort
...
function! log#Error(message, ...) dict abort
...
function! log#Severe(message, ...) dict abort
...
function! log#AddHandler(Handler, ...) abort
  let l:maktaba = Maktaba()
  call ensure#IsCallable(a:Handler)
  let l:fire_recent = ensure#IsBool(get(a:, 1, 0))
  ...
  if l:fire_recent
    ...
    for l:logitem in s:log_queue
      call function#Call(a:Handler, l:logitem)
    endfor
  endif
  return l:remover
endfunction
