...
function! s:GetLeafDir(path) abort
  ...
  let s:cache_leafdirs[a:path] = path#Split(a:path)[-1]
...
function! s:StripTrailingSlash(path) abort
  " Uses path#AsDir to ensure a single trailing slash, then removes it.
  return path#AsDir(a:path)[:-2]
endfunction
...
function! rtp#Split(...) abort
  ...
  if l:paths !=# s:cache_string
    ...
    let s:cache_list = map(
        ...
        \ "path#AsDir(substitute(v:val, s:escaped_char, '\\1', 'g'))")
  endif
...
function! rtp#Join(paths) abort
  call ensure#IsList(a:paths)
  return join(
...
function! rtp#Add(path) abort
  call ensure#IsString(a:path)
  let l:rtp = rtp#Split(&runtimepath)
  let l:end = len(l:rtp) == 0 ? 1 : -1
  let l:path = path#AsDir(a:path)
  call list#RemoveAll(l:rtp, l:path)
  call insert(l:rtp, l:path, 1)
  let l:after = path#AsDir(path#Join([l:path, 'after']))
  if isdirectory(l:after)
    call list#RemoveAll(l:rtp, l:after)
    call insert(l:rtp, l:after, l:end)
  endif
  let &runtimepath = rtp#Join(l:rtp)
...
function! rtp#Remove(path) abort
  call ensure#IsString(a:path)
  let l:rtp = rtp#Split(&runtimepath)
  call list#RemoveAll(l:rtp, path#AsDir(a:path))
  let &runtimepath = rtp#Join(l:rtp)
  let s:cache_string = &runtimepath
  let s:cache_list = l:rtp
endfunction
...
" plugins.
function! rtp#LeafDirs() abort
  let l:plugins = {}
  for l:path in rtp#Split(&runtimepath)
...
" positive/negative.
function! rtp#HasLeafDir(leaf) abort
  return has_key(rtp#LeafDirs(), a:leaf) > 0
endfunction
...
" filetype#Cycle will be called if the plugin has installed any new
" filetypes.
"
" If the plugin is a maktaba plugin, you should use plugin#Install
" instead.
" @throws NotFound if {dir} does not exist.
" @throws BadValue if {dir} does not describe a directory.
function! rtp#Load(dir) abort
  call ensure#IsDirectory(a:dir)
  call rtp#Add(a:dir)
  let l:plugindir = path#Join([a:dir, 'plugin'])
  let l:afterdir = path#Join([a:dir, 'after', 'plugin'])
  ...
  if rtp#DirDefinesFiletypes(a:dir)
    call filetype#Cycle()
  endif
endfunction
...
function! rtp#DirDefinesFiletypes(dir) abort
  return isdirectory(path#Join([a:dir, 'ftdetect']))
      \ || isdirectory(path#Join([a:dir, 'after', 'ftdetect']))
endfunction
