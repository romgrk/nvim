...
" will be emptied (see @function(value#EmptyValue)).


" Promotes {item} to a singleton list, unless it is a list.
function! s:Listify(item) abort
  return value#IsList(a:item) ? a:item : [a:item]
endfunction
...
function! s:StringOrNumeric(value) abort
  ...
  throw error#BadValue(l:msg, a:value)
endfunction
...
function! s:ValueParseError(setting, exception) abort
  let [l:type, l:msg] = error#Split(a:exception)
...
function! setting#SetAtFocus(flag, Oldval, Newval) dict abort
  if empty(self._foci)
    call a:flag.Set(a:Newval)
  else
    call a:flag.Set(value#Focus(a:Oldval, self._foci, a:Newval))
  endif
endfunction
...
function! setting#Empty(flag) dict abort
  let l:Oldval = a:flag.GetCopy()
  try
    let l:Target = value#Focus(l:Oldval, self._foci)
    let l:Default = value#EmptyValue(l:Target)
...
function! setting#Set(flag) dict abort
...
" @dict Setting
" The _Affect function for 'flag+=value' type settings.
function! setting#AddTo(flag) dict abort
  let l:Value = a:flag.GetCopy()
  let l:Target = value#Focus(l:Value, self._foci)
  call ensure#TypeMatchesOneOf(l:Target, [[], {}, '', 0, 0.0])
  if value#IsList(l:Target) && !value#IsList(self._value)
    call add(l:Target, self._value)
    call a:flag.Set(l:Value)
    return
  endif
  if value#IsDict(l:Target) && !value#IsDict(self._value)
    let l:Target[self._value] = 1
    call a:flag.Set(l:Value)
    return
  endif
  if value#IsNumeric(l:Target)
    " Floats and integers can be added indiscriminately.
    call ensure#IsNumeric(self._value)
  else
    " Everything else must be added to something of the same type.
    call ensure#TypeMatches(self._value, l:Target)
  endif
  if value#IsCollection(l:Target)
    call extend(l:Target, self._value)
    call a:flag.Set(l:Value)
  elseif value#IsNumeric(l:Target)
...
" @dict Setting
" The _Affect function for 'flag-=value' type settings.
function! setting#RemoveFrom(flag) dict abort
  let l:Value = a:flag.GetCopy()
  let l:Target = value#Focus(l:Value, self._foci)
  call ensure#TypeMatchesOneOf(l:Target, [[], {}, 0, 0.0])
  if value#IsNumeric(l:Target)
  ...
  if value#IsList(l:Target)
    call map(l:items, 'list#RemoveAll(l:Target, v:val)')
  else
...
" @dict Setting
" The _Affect function for 'flag^=value' type settings.
function! setting#PrependTo(flag) dict abort
  let l:Value = a:flag.GetCopy()
  let l:Target = value#Focus(l:Value, self._foci)
  call ensure#TypeMatchesOneOf(l:Target, ['', []])
  if value#IsList(l:Target)
    let l:items = s:Listify(self._value)
    call map(reverse(l:items), 'insert(l:Target, v:val, 0)')
    call a:flag.Set(l:Value)
  else
    call ensure#IsString(self._value)
...
" @dict Setting
" The _Affect function for 'flag$=value' type settings.
function! setting#AppendTo(flag) dict abort
  call ensure#TypeMatchesOneOf(a:flag.Get(), ['', []])
  call call('setting#AddTo', [a:flag], self)
endfunction
...
" @dict Setting
" The _Affect function for 'flag`=value' type settings.
function! setting#Call(flag) dict abort
  let l:Oldval = a:flag.GetCopy()
  let l:Newval = function#Call(self._value, [l:Oldval], a:flag)
...
" @dict Setting
" The _Affect function for '!flag' type settings.
function! setting#TurnOff(flag) dict abort
...
" @dict Setting
" The _Affect function for '~flag' type settings.
function! setting#Invert(flag) dict abort
  let l:Current = a:flag.GetCopy()
  let l:Target = value#Focus(l:Current, self._foci)
  call ensure#TypeMatches(l:Target, 0)
...
function! s:Setting(definition, flagname, foci, affector, ...) abort
  let l:dict = {
      ...
      \ '_foci': a:foci,
      \ '_GetFlag': function('setting#GetFlag'),
      \ '_SetAtFocus': function('setting#SetAtFocus'),
      \ '_Affect': a:affector,
      \ 'Apply': function('setting#Apply'),
      \}
  if a:0 >= 1
...
function! s:BinaryAffector(operator, value) abort
  if a:operator == '='
    if value#IsString(a:value) && empty(a:value)
      return function('setting#Empty')
    else
      return function('setting#Set')
    endif
  elseif a:operator == '+='
    return function('setting#AddTo')
  elseif a:operator == '-='
    return function('setting#RemoveFrom')
  elseif a:operator == '^='
    return function('setting#PrependTo')
  elseif a:operator == '$='
    return function('setting#AppendTo')
  elseif a:operator == '`='
    return function('setting#Call')
  else
    throw error#Failure('%s wrongly parsed as operator.', a:operator)
  endif
endfunction
...
function! s:UnaryAffector(operator) abort
  if a:operator == '!'
    return function('setting#TurnOff')
  elseif a:operator == '~'
    return function('setting#Invert')
  else
    throw error#Failure('%s wrongly parsed as operator.', a:operator)
  endif
endfunction
...
function! s:ParseValue(chunk) abort
  ...
  if !empty(l:match)
    try
      ...
      throw error#BadValue(l:msg, a:chunk, v:exception)
    endtry
  endif
...
function! s:ParseDictValue(chunk) abort
  ...
  for l:pair in l:pairs
    if empty(l:pair)
      throw error#BadValue('Empty key:value pair in dict %s.', a:chunk)
    endif
...
function! setting#ParseHandle(text) abort
  let l:flagname = matchstr(a:text, s:flagname)
  if empty(l:flagname)
    throw error#BadValue('No flag name in flag handle "%s".', a:text)
  endif
...
" >
"   :echomsg setting#Handle('flag')
"   ~ ['flag', []]
"
"   :echomsg setting#Handle('flag[list][3][val]')
"   ~ ['flag', ['list', 3, 'val']]
" <
...
function! setting#Handle(handle) abort
  let [l:flagname, l:foci, l:rest] = setting#ParseHandle(a:handle)
  if empty(l:rest)
    return [l:flagname, l:foci]
  endif
  let l:msg = 'Junk after flag handle %s. (Unrecognized: %s)'
  throw error#BadValue(l:msg, a:handle, l:rest)
endfunction
...
" |setting#Create()| for details, the only difference is that
...
function! setting#Parse(text) abort
  let [l:unaryop, l:rest] = s:ParseUnaryOp(a:text)
  try
    let [l:flag, l:foci, l:rest] = setting#ParseHandle(l:rest)
  catch /ERROR(BadValue):/
    let l:msg = 'No flag name (or invalid flag prefix) in setting "%s".'
    throw  error#BadValue(l:msg, a:text)
  endtry
  if !empty(l:unaryop)
...
" @throws BadValue if {text} has invalid syntax.
function! setting#Create(text) abort
  let [l:setting, l:rest] = setting#Parse(a:text)
  if empty(l:rest)
    return l:setting
  endif
  throw error#BadValue(
...
function! setting#ParseAll(text) abort
  let l:settings = []
  let l:leftover = a:text
  while !empty(l:leftover)
    let [l:setting, l:leftover] = setting#Parse(l:leftover)
