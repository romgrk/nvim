...
" Used to look up plugins by location in plugin#Install and
" plugin#GetOrInstall.
...
function! s:AlreadyExists(message, ...) abort
  return error#Exception('AlreadyExists', a:message, a:000)
endfunction
...
function! s:CannotEnter(file) abort
  return error#Message(
      \ 'CannotEnter',
      \ 'plugin#Enter must be called from ' .
...
function! s:Fullpath(location) abort
  let l:path = path#AsDir(fnamemodify(a:location, ':p'))
  " Replace trailing path separator with forward slash.
  return path#Dirname(l:path) . '/'
endfunction
...
function! s:ApplySettings(plugin, settings) abort
  ...
  for l:setting in a:settings
    try
      ...
      call add(l:errors, error#Split(v:exception)[1])
    endtry
  endfor
  if !empty(l:errors)
    let l:msg = 'Error configuring %s: %s'
    let l:errtxt = join(l:errors)
    throw error#Message('ConfigError', l:msg, a:plugin.name, l:errtxt)
  endif
  return a:plugin
endfunction
...
function! s:PluginNameFromDir(dir) abort
  ...
  let l:splitpath = path#Split(a:dir)
  if len(l:splitpath) == 0
    throw error#BadValue('Found empty path.')
  endif
  let l:name = plugin#CanonicalName(l:splitpath[-1])
  return l:name
endfunction
...
" Returns the same data structure as rtp#LeafDirs but with
...
function! s:GetUnregisteredLeafdirs() abort
  ...
  if !has_key(l:cache, 'rtp') || !has_key(l:cache, 'plugin_locations') ||
    ...
    let l:leafdirs = {}
    for [l:leafdir, l:leafpath] in items(rtp#LeafDirs())
...
"
" This is used by plugin#Enter, which cares about the TYPE of file
...
""
" @private
" Used by library to help throw good error messages about non-library
" directories.
function! plugin#NonlibraryDirs() abort
...
function! plugin#Enter(file) abort
  let [l:plugindir, l:filedir, l:handle] = s:SplitEnteredFile(a:file)
  let l:plugin = plugin#GetOrInstall(l:plugindir)
...
function! plugin#Detect() abort
  for [l:name, l:location] in items(s:GetUnregisteredLeafdirs())
    call plugin#GetOrInstall(l:location)
  endfor
endfunction
...
function! plugin#RegisteredPlugins() abort
  call plugin#Detect()
...
""
" 1 if {plugin} was registered with plugin#Register.
...
function! plugin#IsRegistered(plugin) abort
  try
    call plugin#Get(a:plugin)
...
function! plugin#CanonicalName(plugin) abort
  " NOTE: This function is performance-sensitive.
...
" >
"   let plugin = plugin#Install(path)
...
" given, they must be a list of maktaba settings (see |setting#Create|).
...
" >
"   let noearly = setting#Parse('instant[earlyfile]=0')
"   let plugin = plugin#Install(path, [noearly])
" <
...
function! plugin#Install(dir, ...) abort
  let l:name = s:PluginNameFromDir(a:dir)
  let l:settings = ensure#IsList(get(a:, 1, []))
...
function! plugin#Get(name) abort
  ...
  for [l:leafdir, l:leafpath] in items(s:GetUnregisteredLeafdirs())
    if plugin#CanonicalName(l:leafdir) is# l:name
      return plugin#GetOrInstall(l:leafpath)
    endif
  endfor

  throw error#NotFound('Plugin %s', a:name)
endfunction
...
" |setting#Create|). If the plugin is new, they will be applied as in
...
" @throws ConfigError if [settings] cannot be applied to this plugin.
function! plugin#GetOrInstall(dir, ...) abort
  let l:name = s:PluginNameFromDir(a:dir)
  let l:settings = ensure#IsList(get(a:, 1, []))
  if has_key(s:plugins, l:name)
...
function! s:CreatePluginObject(name, location, settings) abort
  ...
  let l:plugin = {
      ...
      \ 'flags': {},
      \ 'globals': {},
      \ 'logger': log#Logger(a:name),
      \ 'Source': function('plugin#Source'),
      \ 'Load': function('plugin#Load'),
      \ 'AddonInfo': function('plugin#AddonInfo'),
      \ 'Flag': function('plugin#Flag'),
      \ 'HasFlag': function('plugin#HasFlag'),
      \ 'HasDir': function('plugin#HasDir'),
      \ 'HasFiletypeData': function('plugin#HasFiletypeData'),
      \ 'GenerateHelpTags': function('plugin#GenerateHelpTags'),
      \ 'MapPrefix': function('plugin#MapPrefix'),
      \ 'IsLibrary': function('plugin#IsLibrary'),
      \ 'GetExtensionRegistry': function('plugin#GetExtensionRegistry'),
  ...
  if index(l:rtp_dirs, a:location) == -1 &&
    ...
    call rtp#Add(l:plugin.location)
  endif

  " These special flags let the user control the loading of parts of the plugin.
  if isdirectory(path#Join([l:plugin.location, 'plugin']))
    call l:plugin.Flag('plugin', {})
  endif
  if isdirectory(path#Join([l:plugin.location, 'instant']))
...
function! s:GetSubdirs() dict abort
  if !has_key(self, '_dirs')
    ...
    let l:direct_glob = path#Join([self.location, '*', ''])
    let l:direct_dirs = split(glob(l:direct_glob, 1), "\n")
    let self._dirs = map(l:direct_dirs, 'path#Split(v:val)[-1]')
  endif
  return self._dirs
endfunction
...
function! s:GetAfterSubdirs() dict abort
  if !has_key(self, '_after_dirs')
    " Glob includes trailing slash, which makes glob() only detect directories.
    let l:after_glob = path#Join([self.location, 'after', '*', ''])
    let l:after_dirs = split(glob(l:after_glob, 1), "\n")
    let self._after_dirs = map(l:after_dirs, 'path#Split(v:val)[-1]')
  endif
...
function! s:SourceDir(dir) dict abort
  if has_key(self._entered, a:dir) && value#IsList(self._entered[a:dir])
  ...
  let l:normal = [path#Join([self.location, a:dir]), '^']
  let l:after = [path#Join([self.location, 'after', a:dir]), '$']

  for [l:dir, l:suffix] in [l:normal, l:after]
    if isdirectory(l:dir)
      let l:sources = path#Join([l:dir, '**', '*.vim'])
...
function! s:SourceProblems(plugin, name, path) abort
  if isdirectory(a:path)
    ...
    return error#BadValue(l:msg, a:name, a:plugin)
  endif
  if !filereadable(a:path)
    if path#Exists(a:path)
      let l:msg = 'Cannot source %s in %s. It cannot be read.'
      return error#NotAuthorized(l:msg, a:name, a:plugin)
    endif
    let l:msg = 'Cannot source %s in %s. File does not exist.'
    return error#NotFound(l:msg, a:name, a:plugin)
  endif
  return ''
endfunction
...
function! plugin#Source(file, ...) dict abort
  let l:optional = get(a:, 1)
  let l:name = path#Join(ensure#IsList(a:file))
  let l:path = path#Join([self.location, l:name]) . '.vim'
...
function! plugin#Load(...) dict abort
  " Load specific plugin files.
  if a:0 >= 1
    call ensure#TypeMatchesOneOf(a:1, ['', []])
    let l:files = value#IsList(a:1) ? a:1 : [a:1]
...
function! plugin#GenerateHelpTags() dict abort
  let l:docs = path#Join([self.location, 'doc'])
  ...
  try
    ...
    throw error#Message('Impossible', l:msg, self.name, v:exception)
  endtry
  return 1
endfunction
...
function! plugin#HasDir(dir) dict abort
...
" indent, or syntax).
function! plugin#HasFiletypeData() dict abort
  return rtp#DirDefinesFiletypes(self.location)
endfunction
...
function! plugin#AddonInfo() dict abort
  if !has_key(self, '_addon_info')
    let l:addon_info_path =
        \ path#Join([self.location, 'addon-info.json'])
    try
      ...
      let self._addon_info = json#Parse(l:json)
    catch /E48[45]:/
...
" See @function(setting#Handle) for {flag} syntax.
"
" The following are roughly equivalent: >
"   plugin#Get('myplugin').Flag('foo')
"   plugin#Get('myplugin').flags.foo.Get()
" <
"
...
" syntax: >
"   plugin#Get('myplugin').Flag('plugin[autocmds]')
" <
" This is equivalent to: >
"   plugin#Get('myplugin').flags.plugin.Get()['autocmds']
" <
" This syntax can be chained: >
"   plugin#Get('myplugin').Flag('complex[key][0]')
" <
"
...
" >
"   plugin#Get('myplugin').Flag('foo', 'bar')
"   plugin#Get('myplugin').flags.foo.Set('bar')
" <
"
" Also supports dict flag syntax: >
"   plugin#Get('myplugin').Flag('plugin[autocmds]', 1)
" <
"
" @throws BadValue if {flag} is an invalid flag name.
function! plugin#Flag(flag, ...) dict abort
  let [l:flag, l:foci] = setting#Handle(a:flag)
  if !has_key(self.flags, l:flag)
    if a:0 == 0 || !empty(l:foci)
      let l:msg = 'Flag "%s" not defined in plugin "%s"'
      throw error#NotFound(l:msg, l:flag, self.name)
    endif
    let self.flags[l:flag] = flags#Create(l:flag, a:1)
  elseif a:0 == 0
...
function! plugin#HasFlag(flag) dict abort
...
function! plugin#MapPrefix(letter, ...) dict abort
  let l:throw = ensure#IsBool(get(a:, 1, 0))

  let l:mapfile = path#Join([self.location, 'plugin', 'mappings.vim'])
  if !path#Exists(l:mapfile)
    let l:msg = 'Plugin %s does not have plugin/mappings.vim.'
    throw error#NotFound(l:msg, self.name)
  endif
  ...
  if type(l:prefix) == type(0) && !l:prefix
    ...
    throw error#Message('Unknown', l:msg, self.name)
  endif
  ...
  if !value#IsString(l:prefix) || empty(l:prefix)
    let l:err = error#BadValue(
...
function! plugin#IsLibrary() dict abort
  for l:special in plugin#NonlibraryDirs()
...
" @function(extension#GetRegistry) instead, rather than depend upon
" the plugin directly.
function! plugin#GetExtensionRegistry() dict abort
  return extension#GetInternalRegistry(self.name)
endfunction
